...
ax1 = sns.boxplot(x=df_filtered_uk["endtime"], y=df_filtered_uk["behavior_scale"], color='tab:blue', width=0.4, showfliers=False)
ax1 = sns.lineplot(x=df_grouped_uk.index, y=df_grouped_uk.values, marker='o', linestyle='-', ax=ax1, label='UK Median Behavior Scale', color='b')
ax1.set_title('Behavioral Dynamics over Time in UK')
ax1.set_ylabel('Behavior Scale (UK)')
ax1.set_ylim(0,5)
ax1.legend()

# Plot the line plot for Australia on the second subplot
# ax2 = sns.lineplot(x=df_grouped_uk.index, y=df_grouped_uk.values, marker='o', linestyle='-', label='UK Median Behavior Scale', color='b')
# ax2 = sns.lineplot(x=df_grouped_au.index, y=df_grouped_au.values, marker='o', linestyle='-', label='Australia Median Behavior Scale', color='g')
df_filtered_au.index = df_filtered_au.index.astype(str)
df_grouped_au.index = df_grouped_au.index.astype(str)

ax2 = sns.boxplot(x=df_filtered_au["endtime"], y=df_filtered_au["behavior_scale"], color='tab:orange', width=0.4, showfliers=False)
ax2 = sns.lineplot(x=df_grouped_au.index, y=df_grouped_au.values, marker='o', linestyle='-', ax=ax2, label='Australia Median Behavior Scale', color='g')
ax2.set_title('Behavioral Dynamics over Time in Australia')
ax2.set_xlabel('Time')
ax2.set_ylabel('Behavior Scale (Australia)')
ax2.set_ylim(0,5)
ax2.legend()
